...
"""
        ...
        installed_size = (
            sum(f.stat().st_size for f in app_dir.rglob("*") if f.is_file()) // 1024
        )
...
Maintainer: EyeBrowserOS Team <support@browseros.com>
Description: {ctx.get_app_base_name()} - Privacy-focused web browser
 EyeBrowserOS is an open-source agentic browser that runs AI agents locally.
...
"""
        ...
        run_command(["dpkg-deb", "--build", str(package_dir), str(deb_path)])
...
def get_linux_info() -> Dict[str, str]:
    ...
    info = {"distribution": "unknown", "version": "unknown", "codename": "unknown"}
